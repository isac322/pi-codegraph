...
function normalizeSettings(input: Record<string, unknown>): CodeGraphSettings {
  return {
    autoSync: booleanValue(input.autoSync, defaultSettings.autoSync),
    autoGc: booleanValue(input.autoGc, defaultSettings.autoGc),
    indexStore: path.resolve(
      String(input.indexStore || defaultSettings.indexStore),
    ),
    workerIdleTimeoutMs: integerValue(
      input.workerIdleTimeoutMs,
      defaultSettings.workerIdleTimeoutMs,
      1_000,
    ),
    ...
    syncMinIntervalMs: integerValue(
      input.syncMinIntervalMs,
      defaultSettings.syncMinIntervalMs,
      0,
    ),
    maxOutputChars: integerValue(
      input.maxOutputChars,
      defaultSettings.maxOutputChars,
      4_000,
    ),
    allowedProjectRoots: stringArray(input.allowedProjectRoots).map((root) =>
      path.resolve(root),
    ),
    promptInjection: booleanValue(
      input.promptInjection,
      defaultSettings.promptInjection,
    ),
    codegraphExecutable:
      typeof input.codegraphExecutable === "string"
        ? input.codegraphExecutable.trim()
        : "",
...
export async function loadSettings(
  overrides: Partial<CodeGraphSettings> = {},
): Promise<CodeGraphSettings> {
  const configFile =
    process.env.PI_CODEGRAPH_CONFIG ||
  ...
  try {
    ...
    if (!hasErrorCode(error, "ENOENT"))
      throw new Error(
        `Invalid pi-codegraph config at ${configFile}: ${errorMessage(error)}`,
      );
  }
...
export function settingsEnvironment(
  ...
  return {
    ...
    PI_CODEGRAPH_ALLOWED_ROOTS: settings.allowedProjectRoots.join(
      path.delimiter,
    ),
    PI_CODEGRAPH_PROMPT_INJECTION: String(settings.promptInjection),
    ...(settings.codegraphExecutable
      ? { PI_CODEGRAPH_EXECUTABLE: settings.codegraphExecutable }
      : {}),
  };
}
